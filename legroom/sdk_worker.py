...
from .calibration import CalibrationConfig as _CalibrationConfig, CalibrationController
...
def _handle_compress(request: dict[str, Any]) -> dict[str, Any]:
    ...
    unknown_sdk = sorted(
        set(raw_config) - _CONFIG_FIELDS - _SDK_EXTRA_FIELDS
    )
    if unknown_sdk:
        raise ValueError(
            f"unknown compression config field(s): {', '.join(unknown_sdk)}"
        )
    ...
    ccr_policy = "True" if cfg.ccr_enabled else "False"
    cache_key = cache.key(
        protocol="sdk",
        model=model,
        mode="token",
        messages=messages,
        policy=f"v2:ccr={ccr_policy}",
    )
    ...
    if cached is not None and not shadow_mode:
        ...
        metadata: dict[str, Any] = {"messages": cached.messages}
    ...
    return _response_for_compress(
        request_id, result, store=store, shadow_mode=shadow_mode
    )
...
def _handle_compress_with_store(request: dict[str, Any]) -> dict[str, Any]:
    ...
    cache_key = cache.key(
        ...
        mode="token",
        messages=messages,
        policy=f"v2:ccr=True:store={store_id}",
    )
    cache.put(
        cache_key,
        CachedCompression(
    ...
    return _response_for_compress(
        request_id, result, store=store, shadow_mode=False
    )
...
def _handle_calibration_record(request: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "calibration": {
            ...
            "snapshots": [
                asdict(s) for s in _ensure_state().calibration.snapshots()
            ],
        },
    }
...
def _handle_worker_stats(request: dict[str, Any]) -> dict[str, Any]:
    ...
    elif metrics.total_tokens_before == 0:
        compression_ratio = 0.0
    else:
